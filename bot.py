...
def check_rug_solana(address):
try:
r = requests.get(“https://api.rugcheck.xyz/v1/tokens/” + address + “/report/summary”, timeout=8)
if r.status_code == 200:
data = r.json()
...
if score >= 80:
return “SAFE”, score, risk_names
elif score >= 50:
return “WARN”, score, risk_names
else:
return “DANGER”, score, risk_names
except Exception as e:
log.error(“Rugcheck erreur: %s”, e)
return “UNKNOWN”, 0, []

def check_rug_evm(address):
try:
r = requests.get(“https://api.tokensniffer.com/v2/tokens/” + address, timeout=8)
if r.status_code == 200:
data = r.json()
score = data.get(“score”, 0)
if score >= 70:
...
log.error(“TokenSniffer erreur: %s”, e)
return “UNKNOWN”, 0, []

def get_rug_status(chain, address):
if not address:
return “UNKNOWN”, 0, []
if chain == “solana”:
return check_rug_solana(address)
elif chain in [“bsc”, “ethereum”, “base”]:
return check_rug_evm(address)
return “UNKNOWN”, 0, []
...
def format_signal(pair, score, reasons, rug_status, rug_score, rug_risks):
...
if rug_status == "SAFE":
    rug_emoji = "SAFE"
    rug_line = "RUG CHECK: SAFE (score " + str(rug_score) + "/100)"
elif rug_status == "WARN":
    rug_emoji = "ATTENTION"
    rug_line = "RUG CHECK: ATTENTION (score " + str(rug_score) + "/100)"
elif rug_status == "DANGER":
    rug_emoji = "DANGER"
    rug_line = "RUG CHECK: DANGER (score " + str(rug_score) + "/100)"
else:
    rug_emoji = "?"
    rug_line = "RUG CHECK: Non disponible"

risks_line = ""
if rug_risks:
    risks_line = "Risques: " + ", ".join(rug_risks[:3]) + "\n"

rsn = "\n".join("  - " + r for r in reasons) or "  - Criteres de base atteints"

chain_lower = pair.get("chainId", "").lower()
if chain_lower == "solana":
    rugcheck_url = "https://rugcheck.xyz/tokens/" + addr
    buy_url = "https://jup.ag/swap/SOL-" + addr
elif chain_lower == "bsc":
    rugcheck_url = "https://tokensniffer.com/token/bsc/" + addr
    buy_url = "https://pancakeswap.finance/swap?outputCurrency=" + addr
elif chain_lower == "base":
    rugcheck_url = "https://tokensniffer.com/token/base/" + addr
    buy_url = "https://app.uniswap.org/swap?outputCurrency=" + addr
else:
    rugcheck_url = "https://tokensniffer.com/token/eth/" + addr
    buy_url = "https://app.uniswap.org/swap?outputCurrency=" + addr
...
rug_status, rug_score, rug_risks = get_rug_status(chain, token_addr)
if rug_status == “DANGER”:
log.info(“Token ignore - RUG DANGER: %s”, pair.get(“baseToken”, {}).get(“symbol”, “?”))
sent_tokens.add(addr)
continue
msg = format_signal(pair, score, reasons, rug_status, rug_score, rug_risks)
if send_telegram(msg):
sent_tokens.add(addr)
signal_count += 1
log.info(“Signal #%d envoye: %s score=%d rug=%s”, signal_count, pair.get(“baseToken”, {}).get(“symbol”, “?”), score, rug_status)
